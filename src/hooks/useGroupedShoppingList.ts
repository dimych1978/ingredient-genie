...
import { allMachines, getMachineType, machineIngredients, getIngredientConfig } from '@/lib/data';
...
export const useGroupedShoppingList = () => {
  ...
  const calculateGroupedShoppingList = useCallback(async (
    machineIds: string[],
    dateFrom: Date
    ...
    try {
      ...
      const coffeeIngredients = new Map<string, {
        amount: number;
        unit: string;
        type: string;
      }>();
      ...
      const snackItems = new Map<string, {
        amount: number;
        unit: string;
        name: string;
      }>();
      ...
      allSales.forEach(sale => {
        ...
        const machine = allMachines.find(m => m.id === sale.product_number?.split('-')[0]);
        ...
        if (machineType === 'coffee' && sale.planogram.ingredients) {
          sale.planogram.ingredients.forEach(ingredient => {
            const config = getIngredientConfig(ingredient.name, machine?.model);
            if (config) {
              const key = config.name;
              const current = coffeeIngredients.get(key) || { amount: 0, unit: config.unit, type: config.type };
        ...
        // СНЕКИ и БУТМАТЫ: учитываем как есть
          ...
          const current = snackItems.get(key) || { amount: 0, unit: 'шт', name: sale.planogram.name };
