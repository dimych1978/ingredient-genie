...
import { calculateShoppingList, type SortType } from '@/lib/shopping-calculator';
import type { TelemetronSalesResponse, TelemetronSaleItem, LoadingStatus, LoadingOverrides } from '@/types/telemetron';
import { getLoadingOverrides, saveLoadingOverrides } from '@/app/actions';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from '@/components/ui/card';
import { Loader2, ShoppingCart, Calendar, Download, Check, X, Minus, Save } from 'lucide-react';
...
export const ShoppingList = ({ 
  ...
  const loadShoppingList = useCallback(async () => {
    ...
    try {
      ...
      const machineOverrides: LoadingOverrides = machineIds.length === 1 ? await getLoadingOverrides(machineIds[0]) : {};
      ...
      for (const vmId of machineIds) {
          try {
              ...
              toast({ variant: 'destructive', title: `Ошибка для аппарата ${vmId}`, description: e instanceof Error ? e.message : 'Не удалось загрузить данные.' });
          }
      }
      
      const calculatedList = calculateShoppingList({ data: allSales }, sort, machineOverrides, machineIds[0]);
      ...
      if(calculatedList.length === 0 && allSales.length > 0){
        toast({ variant: 'default', title: 'Нет продаж', description: 'За выбранный период продаж не найдено.' });
      }
      
    } catch (error) {
      console.error('Ошибка загрузки shopping list:', error);
      toast({ variant: 'destructive', title: 'Ошибка', description: error instanceof Error ? error.message : 'Не удалось сформировать список.' });
    } finally {
      setLoading(false);
    }
  }, [machineIds, getSalesByProducts, getMachineOverview, toast, dateFrom, specialMachineDates, sort, forceLoad]);
  ...
  const handleSaveOverrides = async () => {
    if (machineIds.length > 1) {
      toast({ variant: 'destructive', title: 'Ошибка', description: 'Сохранение статусов доступно только для одного аппарата.' });
      return;
    }
    setSaving(true);
    ...
    try {
      ...
      const result = await saveLoadingOverrides({ ...currentOverrides, ...overridesToSave });
      if (result.success) {
        toast({ title: 'Сохранено', description: 'Состояние загрузки ингредиентов успешно сохранено.' });
      } else {
        throw new Error('Не удалось сохранить данные на сервере.');
      }
    } catch (error) {
       toast({ variant: 'destructive', title: 'Ошибка сохранения', description: error instanceof Error ? error.message : 'Неизвестная ошибка.' });
    } finally {
      setSaving(false);
    }
  };
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 p-3 bg-gray-800 rounded-lg">
                  {machineIds.length > 1 &&
                    <div className="md:col-span-2">
                        ...
                        <Input id="vmIds" value={machineIdsString} onChange={handleMachineIdsChange} className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white" placeholder="58690, 40680"/>
                    </div>
                  }
                  <div className="md:col-span-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Input id="dateFrom" type="date" value={format(dateFrom, 'yyyy-MM-dd')} onChange={handleDateChange} className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white"/>
                    </div>
                  </div>
                </div>
                <Button onClick={loadShoppingList} disabled={loading} className="w-full bg-yellow-600 hover:bg-yellow-700 text-white">
        ...
        {shoppingList.length > 0 && (
          <div className="space-y-3">
             <div className="flex gap-3">
               {machineIds.length === 1 && (
                  <Button onClick={handleSaveOverrides} variant="outline" className="border-green-600 text-green-300 hover:bg-green-900/50 flex-1" disabled={saving}>
                    {saving ? <Loader2 className="animate-spin mr-2 h-4 w-4" /> : <Save className="mr-2 h-4 w-4" />}
                    Сохранить состояние
                  </Button>
               )}
               <Button onClick={downloadList} variant="outline" className="border-gray-600 text-gray-300 hover:bg-gray-800 flex-1">
            ...
            <div className="grid gap-2">
              ...
              {shoppingList.map((item, index) => (
                <div key={index} className="flex justify-between items-center p-3 bg-gray-800 border border-gray-700 rounded-lg">
                  <div className="flex-1 space-y-1">
                    <div className="font-medium">{item.name}</div>
                    <div className="text-base text-green-400">
                      Нужно: <span className="font-bold">{item.amount.toLocaleString('ru-RU')}</span> {item.unit}
                    </div>
                  </div>
                  <div className="flex items-center gap-1">
                      <Tooltip>
                          <TooltipTrigger asChild>
                              <Button variant="ghost" size="icon" className={cn("rounded-full", item.status === 'full' && "bg-green-500/20 text-green-400")} onClick={() => handleStatusChange(index, 'full')}>
                      ...
                      <Tooltip>
                          <TooltipTrigger asChild>
                              <Button variant="ghost" size="icon" className={cn("rounded-full", item.status === 'none' && "bg-red-500/20 text-red-400")} onClick={() => handleStatusChange(index, 'none')}>
                                  <X className="h-5 w-5"/>
                              </Button>
                          </TooltipTrigger>
                      ...
                      <Tooltip>
                          <TooltipTrigger asChild>
                              <Button variant="ghost" size="icon" className={cn("rounded-full", item.status === 'partial' && "bg-yellow-500/20 text-yellow-400")} onClick={() => handleStatusChange(index, 'partial')}>
