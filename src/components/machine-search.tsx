...
function Unavailabled () {
  return (<h1 className='text-yellow-200 text-8xl'>АЙ-АЙ-АЙ</h1>)
}

export function MachineSearch() {
  const [open, setOpen] = useState(false);
  const [value, setValue] = useState<string | null>(null);
  const [showUnavailable, setShowUnavailable] = useState(false); // Добавляем состояние для отображения предупреждения
...
//    }, 3000);
  ...
  return (
    <>
      {/* Показываем компонент, если showUnavailable = true */}
      {showUnavailable && <Unavailabled />}
